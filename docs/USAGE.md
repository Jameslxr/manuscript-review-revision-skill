...
采用中立通用稿件方案；按严格通用生物医学标准先审稿，不绑定期刊。
```

中立路线不查询或混合期刊规则，不生成录用尺度卡，也不声称投稿合规；
选定目标期刊后必须重新打开期刊 profile、format plan 和受影响的审稿门。
...
目标期刊确定后，完整审稿会同时建立
`01b_acceptance_tolerance_card.json`：优先阅读至少 5 篇近期同刊同类型录用论文，
区分官网硬性规则、科学有效性底线、竞争力预期、已被接受的固有局限和可选增强。

每次还会重新读取该期刊、文章类型和投稿阶段的当前官网要求，生成
`01a_journal_format_plan.json`。该计划逐项记录字体、字号、行距、真实空段落、
页边距、匿名化、连续行号、连续动态页码、section 顺序、字数与图表限制、引用格式、声明、
...
2. 检查目标期刊格式；
3. 将 Claim 分为 A（核心物质性）、B（支持性）和 C（普通背景）；
4. 对 A/B 逐条核查，对 C 进行真实性检查和有记录的语义抽查；
5. 判断每条已审计引用是直接、部分、背景、矛盾或无法评估。
```
...
无论使用哪个宿主，所有审稿角色都必须接收相同稿件哈希、期刊档案和事实材料，并且在初审完成前看不到其他角色的结论。Panel 固定 5 个核心席位，最多增加 1 个有明确风险触发原因的专项席位；每个审稿轴只有 1 个主责角色，每席最多 8 个优先问题和 1,800 个等效词单位。每个 reviewer 还必须记录唯一任务 ID、运行时间、输入哈希和报告哈希；缺少任务身份时，Panel 标记为 `NOT ASSESSABLE`。

独立报告完成后，系统建立 `reviews/concern_ledger.tsv`。每条意见必须指向
原文位置和证据位置，并给出可观察的解决标准。只有至少两个独立 reviewer
提出同一问题时才能标记为共识；冲突意见不得被多数票抹平。

每条意见同时归为：致命有效性缺陷、投稿前可修正问题、可接受的固有局限或
可选增强。只有在准确收窄 Claim 并透明披露局限后，中心稿件仍然无法成立时，
才允许标记为 `BLOCKING`。额外队列、实验或分析如果只是增强竞争力，不得写成
投稿前强制要求。
...
- 目标期刊未确定；
- 关键稿件版本冲突；
- 无法执行至少 5 个独立 Agent 任务（工作流保证标记为 `NOT ASSESSABLE`，不等同于稿件科学失败）；
- 无法取得 reviewer 任务 ID/执行日志，或报告哈希无法闭环（同样属于工作流保证问题）；
- 期刊强制规则无法核实；
- 核心或支持性参考文献只有 metadata，没有足以判断 Claim 支持的内容；
- 作者尚未授权修改；
